...
private ["_activeControl","_bg","_btn_arr","_buffer","_button_gen","_cfgItemInteractions","_config","_control","_data","_display","_interactActions","_interactedItem","_magCount","_missionconfig","_pos","_start_idc","_type","_y2d"];
//[[[end]]]
...
if !(_interactedItem isEqualTo []) then {
    _data = _interactedItem select 1;

    // load action configs
    _cfgItemInteractions = (configfile >> "CfgItemInteractions" >> _data);
    _missionconfig = (missionConfigFile >> "CfgItemInteractions" >> _data);
    ...
    if !(_interactActions isEqualTo []) then {
        ...
        _start_idc = 12346;
        {
            ...
            _button_gen ctrlAddEventHandler ["ButtonClick",_btn_code + "(uiNamespace getVariable ['EPOCH_active_button_control', controlNull]) ctrlShow false;"];
            _y2d = _y2d + 0.0625;
