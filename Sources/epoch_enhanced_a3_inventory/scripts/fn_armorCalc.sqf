/*
    Epoch Enhanced Arma3 Inventory - Gear armor calculation functions
    ...
    Example:
    _vestArmor = ["gear",(vest player)] call EPOCH_fnc_armorCalc;
...
switch _type do {
    ...
    case "selectItem": {
        ...
        uiNamespace setVariable ["EPOCH_InteractedItem",[_text,_data,_pic,_val]];
    };
    case "init": {
        _item params ["_unit","_targetContainer","_secondaryContainer"];
        EPOCH_targetContainer = _targetContainer;
        EPOCH_secondaryContainer = objNull;
        if !(isNull _secondaryContainer) then {
            EPOCH_targetContainer = _secondaryContainer;
            EPOCH_secondaryContainer = _targetContainer;
        };
        ...
        if (!isNull findDisplay 602) then {
            ...
            // init custom sub menu handler
            {
                ...
                _container ctrlAddEventHandler ["LBDblClick",format["[_this,%1] call EPOCH_fnc_subMenu;",_x]];
            } forEach [619,633,638];
            // armor stats init
            {
                _remoteContainer = _display displayCtrl _x;
                _remoteContainer ctrlAddEventHandler ["LBDrag",format["['selectItem',_this,%1] call EPOCH_fnc_armorCalc;['refresh'] call EPOCH_fnc_armorCalc;",_x]];
                _remoteContainer ctrlAddEventHandler ["LBSelChanged",format["['selectItem',_this,%1] call EPOCH_fnc_armorCalc;['refresh'] call EPOCH_fnc_armorCalc;",_x]];
            } forEach [632,640];
    ...
    case "refresh": {
        ...
        if (!isNull findDisplay 602) then {
          ...
          if (isNil "EPOCH_MAX_ARMOR") then {
              ["maxArmorInit"] call EPOCH_fnc_armorCalc;
          };
          _totalArmor = 0;
          ...
          _uniformArmor = ["uniform",(uniform player)] call EPOCH_fnc_armorCalc;
          ...
          if (_selectedItem == 801) then {
              _newArmor = _newArmor + (["uniform",_selectedClass] call EPOCH_fnc_armorCalc);
          } else {
          ...
          if (_selectedItem == 701) then {
              _newArmor = _newArmor + (["gear",_selectedClass] call EPOCH_fnc_armorCalc);
          } else {
              _newArmor = _newArmor + _vestArmor;
          };
          _bar = uiNameSpace getVariable "RscCustomProgressHeadgear";
          _headgearArmor = ["gear",(headgear player)] call EPOCH_fnc_armorCalc;
